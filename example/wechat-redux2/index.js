...
var regeneratorRuntime$1 = _interopDefault(require('@babel/runtime/regenerator'));
...
var ActionTypes = {
  INIT: "@quick-redux/INIT".concat(randomString()),
  REPLACE: "@quick-redux/REPLACE".concat(randomString()),
  PROBE_UNKNOWN_ACTION: function PROBE_UNKNOWN_ACTION() {
    return "@quick-redux/PROBE_UNKNOWN_ACTION".concat(randomString());
  }
};
...
function createStore(reducer, preloadedState, enhancer) {
  ...
  if (typeof enhancer !== 'undefined') {
    ...
    return setStore(enhancer(createStore)(reducer, preloadedState));
  }
  ...
  function dispatch(action) {
    ...
    try {
      isDispatching = true;
      setCurrentState(currentReducer(getCurrentState(), action));
    } finally {
      isDispatching = false;
    }
...
var listenDispatch = function listenDispatch(methods) {
...
function connect(mapState, mapDispatch) {
  // 初始状态
  var state = mapState ? mapState(store.getState()) : {};
  /***
   * 每次 dispatch
   * 都应该被处理
   */

  var methods = listenDispatch(mapDispatch ? mapDispatch(store.dispatch) : {});
  return function (conf) {
    var computedConf = Object.assign(_objectSpread2({}, methods, {}, conf, {
      onInit: function onInit() {
        ...
        var sub = function sub() {
          // 每次执行完成执行更新操作
          var newState = mapState ? mapState(store.getState()) : {};
          if (!newState) return;
        ...
        sub();
        return conf.onInit && (_conf$onInit = conf.onInit).call.apply(_conf$onInit, [this].concat(Array.prototype.slice.call(arguments)));
      }
    })); // 使用新特性

    if (computedConf["private"]) {
      computedConf["protected"] = _objectSpread2({}, state, {}, conf["private"]);
    } else {
      if (!conf.data) conf.data = {};
      computedConf.data = _objectSpread2({}, state, {}, conf.data);
    }
...
var injectRef$1 = Object.getPrototypeOf(global) || global;
injectRef$1.regeneratorRuntime = require('@babel/runtime/regenerator');
var createKey = function createKey(type) {
  return "action:".concat(type);
}; // 创建本地缓存

var createLocalStorage = function createLocalStorage() {
  var storage = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
  return function () {
    injectRef$1.storage = storage;
    return function (next) {
      return function (action) {
        var isStorage = storage.find(function (item) {
  ...
  return regeneratorRuntime.async(function initStorage$(_context) {
    while (1) {
      switch (_context.prev = _context.next) {
        case 0:
          store = _args.length > 0 && _args[0] !== undefined ? _args[0] : [];
          storageAPI = _args.length > 1 && _args[1] !== undefined ? _args[1] : storageAPIDefault;
          storage = injectRef$1.storage;
          i = 0;
 ...
 */
var index = {
  regeneratorRuntime: regeneratorRuntime$1,
...
exports.regeneratorRuntime = regeneratorRuntime$1;
